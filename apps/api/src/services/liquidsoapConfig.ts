...
const CONFIG_PATH =
  ...
  join(process.cwd(), '..', '..', 'liquidsoap', 'config.json');

const SCRIPT_PATH =
  process.env.LIQUIDSOAP_SCRIPT ||
  join(process.cwd(), '..', '..', 'liquidsoap', 'radio.liq');
...
function renderScript(config: LiquidsoapConfig, icecastSourcePassword: string): string {
  ...
  lines.push('# Generated by Radio app — DO NOT EDIT (use Settings → LiquidSoap, or Raw Script for advanced).');
  lines.push('');
