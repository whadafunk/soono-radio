...
1. **Maintain a continuously-updated pointer**, `nextHardSegment: { segment, wall_clock_start_ms }`, via a new forward-scanning resolver (distinct from `resolveCurrentSegment`, which only ever answers "what covers timestamp X") that walks the clock's segment list — across hour-instance boundaries where necessary — starting just after the tracked position, stopping at the first segment whose `start_policy.type === 'hard'`. Null if none exists within a bounded horizon (e.g. the next few clock instances) — the mechanism no-ops entirely in that case, identical to today's behavior.

2. **Recompute cheaply, not on every tick from scratch.** The pointer only needs to change when the tracked position advances past it or when the schedule underneath it changes (calendar/template/clock edit — the same invalidation hook Decision 54 already earmarked for auto-triggering reconcile on mutation, still deferred). A per-tick DB-free comparison against the cached pointer keeps this from adding meaningful load to the 500ms loop.

3. **Two comparisons, both against the maintained pointer, run alongside the existing single-step `maybeHandleHardStartGate`:**
   - If `nextHardSegment` **is** the segment `maybeHandleHardStartGate` already checks (the tracked "current+1"), nothing changes — existing fill/trim logic (Decision 43, 49) handles it exactly as today.
   - If `nextHardSegment` **is not** the tracked "current+1" segment, the tracked plan chain has fallen behind by more than one segment. This is the new case: the segment(s) between tracked position and `nextHardSegment` must be skipped outright, and a plan for `nextHardSegment` activated directly.

4. **Skipping reuses Align to Clock's forward-only invalidation, not a new mechanism.** Whatever plan is currently tracked as `next_plan_id` (drafted for a now-bypassed segment) gets invalidated the same way Decision 56 invalidates a stale active plan — transitioned out of `active`/`draft` status — and a fresh draft is requested directly for `nextHardSegment`'s `segment_id`, funneled through the same `activatePlanById` path Decision 61 already made authoritative for playhead bookkeeping. This is, in effect, "Align to Clock targeted at a specific future segment" rather than "align to wall-clock now" — same forward-only guarantee (content in the skipped segments is deliberately not aired, an accepted trade-off exactly as Decision 56 already accepts for the operator-triggered case), just triggered automatically instead of by a button click.

5. **Skipped segments get logged explicitly**, not silently absorbed — a `SEGMENT_SKIPPED` event per bypassed segment id, so operators and any future reporting can see what was deliberately dropped to make a hard boundary, rather than inferring it from an absence of `plan_items` rows (today's only trace).

6. **Interaction with Decision 57's trust criteria:** this path does not go through the trust check — by construction, once the divergence in step 3 is detected, the tracked chain is already known to be wrong, so there's nothing to "trust or not," only to rebuild. This mirrors how Align to Clock forces a rebuild regardless of what the trust criteria would otherwise conclude.

**Deliberately out of scope for this decision:** what happens if two hard-start segments occur back-to-back with no flexible segment between them (the single-step gate already covers the immediate one; the lookahead pointer would simply point at the *next* one after that, unaffected). Also out of scope: any UI surfacing of the new `SEGMENT_SKIPPED` event beyond the existing log — revisit once this is built and lived with.

---

### Decision 63 — Persist on-air/cut-short outcome; exclude incomplete plays from billing-facing counters
...
4. **Update every counter that currently reads `play_history` by `campaign_id` without regard to completeness** — `campaign.ts:countPlaysByCampaign`, `spotBudget.ts`'s pacing/demand functions — to add `aborted = false` to their filter. This is the only behavior change these functions need; no new columns are required on their query side.
5. **Schema-wise, no new column is required** — `play_history.aborted` already exists; this decision is entirely about giving it a real writer and reader. If a later decision wants richer detail (e.g. persisting `on_air` on `plan_items` itself, for symmetry with the log field, or recording actual elapsed seconds played), that's additive and can follow `drizzle-kit generate` normally — not needed to close the billing-accuracy gap itself.

**Critical scoping constraint, confirmed while reviewing this against the content-process protocol (Decision 11/48):** the `aborted = false` filter in item 4 must be added **only** to the billing/pacing-cap counters (`campaign.ts`'s `countPlaysByCampaign`, `spotBudget.ts`'s pacing/demand functions) — it must **not** be applied to any least-recently-played (LRP) "last played" query, such as `music.ts`'s rotation-position lookups or `branding.ts`'s `lrpPlaylist` (`MAX(started_at)` grouped by `media_id`, no existing `aborted` filter). For rotation/cooldown purposes, a track that aired for even a few seconds before being cut is legitimately "just used" and must still be deprioritized from immediate reuse — only for billing/pacing-cap purposes should it count as zero. Applying the filter uniformly would make a cut-short item look never-played to LRP logic and cause it to be picked again immediately, the opposite of the intended fix.

**Why this surfaced now:** confirmed while checking whether jingle/envelope content (routed entirely through the single Branding process, not a dedicated per-content-type process — see Decision 48) needs the same billing treatment. It doesn't: Branding has no daily-cap, pacing, or billing concept anywhere in its code — jingles and envelopes are pure LRP sequencing. So Decision 63's fix is correctly scoped to Campaign's billing-relevant counters only; Branding needs no changes, but the LRP-query exclusion above must be stated explicitly so a future implementer doesn't reuse the same `aborted` filter in the wrong place by analogy.

**Deliberately out of scope for this decision:** partial-credit billing models, surfacing cut-short indicators in the Supervisor UI's recent-plays list, and any retroactive backfill of historical `play_history` rows (the fix is forward-looking only — nothing tries to reconstruct `aborted` for plays that already happened before this ships).
